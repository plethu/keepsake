...
async fn caller_transaction_replay_fencing_and_deadline() -> TestResult<()> {
    ...
    let tenant = SqliteHarness::tenant()?;
...
async fn verify_terminal_receipts(
    repo: &keepsake_sqlx::TenantSqlxKeepsakeRepository<'_, keepsake_sqlx::SqliteBackend>,
    ...
    let conflicting = repo.apply(&changed).await;
    assert!(
        matches!(conflicting, Err(RepositoryError::CommandConflict)),
        "{conflicting:?}"
    );
...
async fn absent_observation_fences_a_concurrent_apply() -> TestResult<()> {
    let path = env::temp_dir().join(format!("keepsake-concurrency-{}.sqlite", Uuid::now_v7()));
    ...
    let pool = SqlitePoolOptions::new()
        ...
        .await?;
    let root = keepsake_sqlx::SqliteKeepsakeRepository::new(
        pool.clone(),
        "https://tests.invalid/concurrent",
    )?;
    root.migrate().await?;
    sqlx::raw_sql(
...
#[tokio::test]
async fn transaction_projection_and_expiry_reject_wrong_schema_track() -> TestResult<()> {
    let (repo, pool) = SqliteHarness::repo().await?;
    sqlx::query("update keepsake_schema_metadata set value = '3' where key = 'api_track'")
        .execute(&pool)
        .await?;
    let at = ts("2026-01-01T00:00:00Z")?;
    let mut tx = pool.begin().await?;
    let revoke = keepsake::RevokeKeepsake::new(
    ...
    assert!(matches!(
        repo.revoke_in_transaction(&mut tx, &revoke).await,
        Err(RepositoryError::BackendMismatch { .. })
    ));

    assert!(matches!(
        repo.fulfillment_snapshot_in_transaction(&mut tx, Uuid::now_v7())
            .await,
        Err(RepositoryError::BackendMismatch { .. })
    ));
    assert!(matches!(
        repo.expire_due_timed_in_transaction(&mut tx, at, 10).await,
        Err(RepositoryError::BackendMismatch { .. })
    ));
    assert!(matches!(
        repo.expire_due_fulfilled_in_transaction(&mut tx, at, 10)
            .await,
        Err(RepositoryError::BackendMismatch { .. })
    ));
    tx.rollback().await?;
