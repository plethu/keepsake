...
#[tokio::test]
async fn sqlite_identifier_upgrade_preserves_terminal_history_and_receipt() -> TestResult<()> {
    let pool = sqlite_v3_pool().await?;
    sqlx::query("insert into keepsake_relation_definitions (tenant_id,id,kind,key,enabled,expiry_policy,created_at,updated_at) values ('tenant-upgrade','00000000-0000-0000-0000-000000000001','block','social',1,'{\"type\":\"manual_only\"}','2026-01-01T00:00:00.000000Z','2026-01-01T00:00:00.000000Z')")
        .execute(&pool).await?;
    sqlx::query("insert into keepsakes (tenant_id,id,subject_kind,subject_id,relation_id,state,expiry_policy,applied_at,expires_at,fulfilled_at,revoked_at,metadata,created_at,updated_at) values ('tenant-upgrade','00000000-0000-0000-0000-000000000002','directed-pair','a:b','00000000-0000-0000-0000-000000000001','revoked','{\"type\":\"manual_only\"}','2026-01-01T00:00:00.000000Z',null,null,'2026-01-01T00:00:00.000000Z','{\"origin\":\"published-v3\"}','2026-01-01T00:00:00.000000Z','2026-01-01T00:00:00.000000Z')")
        .execute(&pool).await?;
    let root =
        SqliteKeepsakeRepository::new(pool.clone(), "https://tests.invalid/identifier-upgrade")?;
