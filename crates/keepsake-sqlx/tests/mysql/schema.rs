...
async fn catalog_check_rejects_changed_column_index_and_constraint() -> TestResult<()> {
    let _database_url = env::var("MYSQL_DATABASE_URL")?;
    ...
    reject_changed_subject_column(&pool, &repo).await?;

    reject_incomplete_subject_index(&pool, &repo).await?;

    reject_incomplete_or_reordered_unique_index(&pool, &repo).await?;

    reject_wrong_foreign_key_target(&pool, &repo).await?;

    reject_foreign_key_update_cascade(&pool, &repo, &version).await?;
...
async fn reject_changed_subject_column(
    pool: &MySqlPool,
    repo: &MySqlKeepsakeRepository,
) -> TestResult<()> {
    sqlx::query(
...
async fn reject_incomplete_subject_index(
    pool: &MySqlPool,
    repo: &MySqlKeepsakeRepository,
) -> TestResult<()> {
...
async fn reject_incomplete_or_reordered_unique_index(
    pool: &MySqlPool,
    repo: &MySqlKeepsakeRepository,
) -> TestResult<()> {
...
async fn reject_wrong_foreign_key_target(
    pool: &MySqlPool,
    repo: &MySqlKeepsakeRepository,
) -> TestResult<()> {
...
async fn reject_foreign_key_update_cascade(
    pool: &MySqlPool,
    repo: &MySqlKeepsakeRepository,
    version: &str,
) -> TestResult<()> {
    ...
    // has been observed.
    remove_tenant_checks_for_update_cascade(pool, version).await?;
    sqlx::query(
...
async fn remove_tenant_checks_for_update_cascade(
    pool: &MySqlPool,
    version: &str,
) -> TestResult<()> {
    ...
    sqlx::query(drop_tenant_nonempty_check)
        .execute(pool)
        .await?;
    sqlx::query(drop_counter_tenant_check).execute(pool).await?;
