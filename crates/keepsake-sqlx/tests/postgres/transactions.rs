...
#[tokio::test]
#[ignore = "requires disposable PostgreSQL database"]
...
async fn verify_terminal_receipts(
    repo: &keepsake_sqlx::TenantSqlxKeepsakeRepository<'_, keepsake_sqlx::PostgresBackend>,
...
async fn absent_observation_fences_a_concurrent_apply() -> TestResult<()> {
    ...
    let mut unsupported = pool.begin().await?;
    sqlx::query("set transaction isolation level repeatable read")
        .execute(&mut *unsupported)
        .await?;
    assert!(matches!(
    ...
    assert!(matches!(
        repo.revoke_in_transaction(&mut unsupported, &revoke).await,
        Err(RepositoryError::UnsupportedIsolation)
    ));
