    ...
    C: RelationCache,
{
    ...
    pub async fn apply(&self, command: &ApplyKeepsake) -> RepositoryResult<AppliedKeepsake> {
        ...
        let (applied, event) = apply_tx(&mut tx, command).await?;
...
pub(super) async fn apply_tx(
    tx: &mut Transaction<'_, Postgres>,
    command: &ApplyKeepsake,
) -> RepositoryResult<(AppliedKeepsake, keepsake::AuditEvent)> {
    let relation = relation_for_share_tx(tx, command.relation_id).await?;
    ...
    let applied = sqlx::query_as::<_, AppliedKeepsakeWriteRow>(
        ...
        select
            $1, $2, $3, r.id, 'applied', r.expiry_policy, $4,
            case when r.expiry_policy->>'type' = 'at'
                 then (r.expiry_policy->>'timestamp')::timestamptz else null end,
            $5, $4, $4
