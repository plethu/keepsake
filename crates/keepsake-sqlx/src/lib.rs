...
//! deduplicate at the tenant-scoped Dovecote `(tenant_id, source, event_id)`
//! boundary. If events are projected to `CloudEvents` for transport, preserve
//! the tenant routing value alongside the `CloudEvents` `(source, id)` pair.
//!
...
//! [`SqlxKeepsakeRepository::check_schema`] before serving requests. The root
//! repository creates explicit tenant views with [`SqlxKeepsakeRepository::for_tenant`];
//! ordinary relation and lifecycle operations are unavailable until that scope
//! is selected. The
...
pub mod prelude {
    ...
    pub use crate::{
        AdminKeepsakeRepository, KeepsakeRepository, PostgresKeepsakeRepository,
        TenantKeepsakeRepository, TimedKeepsakeRepository,
    };
    #[cfg(feature = "mysql")]
...
#[cfg(feature = "postgres")]
pub use repository::{
    AdminKeepsakeRepository, KeepsakeRepository, PostgresBackend, PostgresKeepsakeRepository,
    TenantKeepsakeRepository, TimedKeepsakeRepository, TimedTenantSqlxKeepsakeRepository,
};
#[cfg(feature = "cache")]
