...
`KeepsakeRepository` is generic over the `RelationCache` trait and defaults to
`NoopRelationCache`, so callers do not pay for a cache they did not configure.
Applications that need cross-pod invalidation should provide their own cache
adapter.

Active subject lookups and membership scans stay application-owned cache
decisions. Put those policies in an application wrapper keyed by bounded values
such as relation id, relation kind/name, subject kind/id, cursor, limit, and any
application partition scope.
