...
blob and easier for downstream export/indexing tools to reason about. If a
field becomes part of a strict application reporting contract, promote it in the
application's audit sink or warehouse schema rather than asking Keepsake to own
...
SQL append-only audit, Kafka, object storage, and event-store integrations
should be adapters. Those adapters should preserve Keepsake's durable audit
fields and append application context attributes alongside them, while keeping
logging and metrics separate.
